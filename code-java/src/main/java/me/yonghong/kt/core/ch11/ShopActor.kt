package me.yonghong.kt.core.ch11
...
sealed class Action {
    data class BuyOrInit(
        val id: Long,
        val userId: Long,
        val amount: Long,
        val shopName: String,
        val stocks: Map<Long, Int>
    ) : Action()
...
class ManageActor : UntypedAbstractActor() {
    override fun onReceive(message: Any?) {
        when (message) {
            is Action.BuyOrInit -> getOrInit(message.shopName, message.stocks).forward(
                Action.Buy(
                    message.id,
                    message.userId,
                    message.amount
            ...
            is Action.GetStockOrInit -> getOrInit(message.shopName, message.stocks).forward(
                Action.GetStock(message.id),
                context
            )
        }
    }
    ...
    fun getOrInit(shopName: String, stocks: Map<Long, Int>): ActorRef {
        return context.findChild("shop-actor-${shopName}")
            .orElseGet { context.actorOf(Props.create(ShopActor::class.java, stocks), "shop-actor-${shopName}") }
    }
