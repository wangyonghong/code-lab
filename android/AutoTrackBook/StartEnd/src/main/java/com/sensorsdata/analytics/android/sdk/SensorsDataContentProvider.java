...
public class SensorsDataContentProvider extends ContentProvider {
    ...
    public boolean onCreate() {
        if (getContext() != null) {
            ...
            uriMatcher.addURI(packName + ".SensorsDataContentProvider",
                SensorsDataTable.APP_STARTED.getName(), APP_START);
            uriMatcher.addURI(packName + ".SensorsDataContentProvider",
                SensorsDataTable.APP_END_STATE.getName(), APP_END_STATE);
            uriMatcher.addURI(packName + ".SensorsDataContentProvider",
                SensorsDataTable.APP_PAUSED_TIME.getName(), APP_PAUSED_TIME);
            sharedPreferences = getContext().getSharedPreferences("com.sensorsdata.analytics" +
                ".android.sdk.SensorsDataAPI", Context.MODE_PRIVATE);
    ...
    public Cursor query(@NonNull Uri uri, @Nullable String[] strings, @Nullable String s,
        ...
        switch (code) {
            case APP_START:
                int appStart = sharedPreferences.getBoolean(SensorsDatabaseHelper.APP_STARTED,
                    true) ? 1 : 0;
                matrixCursor = new MatrixCursor(new String[]{SensorsDatabaseHelper.APP_STARTED});
                matrixCursor.addRow(new Object[]{appStart});
                break;
            case APP_END_STATE:
                int appEnd = sharedPreferences.getBoolean(SensorsDatabaseHelper.APP_END_STATE,
                    true) ? 1 : 0;
                matrixCursor = new MatrixCursor(new String[]{SensorsDatabaseHelper.APP_END_STATE});
                matrixCursor.addRow(new Object[]{appEnd});
                break;
            case APP_PAUSED_TIME:
                long pausedTime = sharedPreferences.getLong(SensorsDatabaseHelper.APP_PAUSED_TIME
                    , 0);
                matrixCursor =
                    new MatrixCursor(new String[]{SensorsDatabaseHelper.APP_PAUSED_TIME});
