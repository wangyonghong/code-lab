...
/*public*/ class SensorsDataPrivate {
    ...
    private static void delegateViewAccessibilityDelegate(final Context context, final View view) {
        ...
        if (view instanceof RatingBar) {
            ...
            try {
                ...
                //Child
                Field mOnChildClickListenerField = viewClazz.getDeclaredField(
                ...
                //Group
                Field mOnGroupClickListenerField = viewClazz.getDeclaredField(
    ...
    private static SeekBar.OnSeekBarChangeListener getOnSeekBarChangeListener(View view) {
        try {
            ...
            Field mOnCheckedChangeListenerField = viewClazz.getDeclaredField(
    ...
    public static void trackAdapterView(AdapterView<?> adapterView, View view, int groupPosition,
                                        int childPosition) {
        try {
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("$element_type", adapterView.getClass().getCanonicalName());
            jsonObject.put("$element_id", getViewId(adapterView));
            if (childPosition > -1) {
                jsonObject.put("$element_position", String.format(Locale.CHINA, "%d:%d",
                    groupPosition, childPosition));
            } else {
                jsonObject.put("$element_position", String.format(Locale.CHINA, "%d",
                    groupPosition));
            }
            StringBuilder stringBuilder = new StringBuilder();
            String viewText = traverseViewContent(stringBuilder, view);
